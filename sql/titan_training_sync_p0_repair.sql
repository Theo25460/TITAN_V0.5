...
-- TITAN OS - P0 repair for sport session storage.
-- Purpose: make training_logs + titan_submit_training_session reliable for public release.
-- Safe to rerun. Does not delete training history.
...
revoke all on function public.titan_training_storage_health() from public;
grant execute on function public.titan_training_storage_health() to authenticated;
