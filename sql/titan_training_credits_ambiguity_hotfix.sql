...
-- TITAN OS - Hotfix DB-only for "column reference credits is ambiguous".
-- Apply this if saving a sport session fails inside titan_submit_training_session.
-- No deploy required. Safe to rerun.
...
begin
  ...
  values (
    ...
    v_xp,
    coalesce(p_date, now()),
    v_details || jsonb_build_object('serverReward', true, 'serverVersion', 'public-release-v1.1')
  )
